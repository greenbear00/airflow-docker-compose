...
@Job(job_name="UserSummaryRunner")
def runner(target_date:datetime, logger:JtbcLogger, my_helper:Helper):
    ...
    logger.info(
        f"started job(UserSummaryRunner) > target_date = {target_date.strftime('%Y-%m-%d %H:%M:%S')}")

    try:
        start = time.time()

        user_summary = UserSummary(helper=my_helper, level=my_helper.build_level)
        user_summary.do_user_summary_info_by_daily(the_date=target_date)
        user_summary.do_user_summary_info_by_montly(the_date=target_date)

        end = time.time()
        logger.info(f"taken time > {end - start}")
        logger.info(
            f"job(UserSummaryRunner) was done. (target_date = {target_date.strftime('%Y-%m-%d %H:%M:%S')})\n\n\n")
